...
DEVICE="${DEVICE:-cuda}"
ARCHIVE="true-active-setwise-multiseed-results.tar.gz"
...
if [[ ! -d "${OUTPUT_DIR}/per_query" ]]; then
  echo "Missing corrected seed-42 results at ${OUTPUT_DIR}" >&2
  exit 1
fi

run_experiment() {
  python "${EXP_DIR}/run_true_active_setwise_extra_seeds.py" \
    --model "${MODEL}" \
