...
DEVICE="${DEVICE:-cuda}"
ARCHIVE="listwise-diagnostic-results.tar.gz"
...
rm -rf "${EXP_DIR}/results/listwise_diagnostic"

python "${EXP_DIR}/diagnose_listwise.py" \
  --model "${MODEL}" \
  --model-revision "${MODEL_REVISION}" \
  --device "${DEVICE}"

tar -czf "${ARCHIVE}" "${EXP_DIR}/results/listwise_diagnostic"
