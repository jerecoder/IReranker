...
class MohajerRanker(Ranker):
    ...
    def rank(self, task: RankingTask) -> List[int]:
        ...
        for _ in range(K):
            ...
            groups[champ_og_group] = [idx for idx in groups[champ_og_group] if idx != best_item_idx]
        ...
        # print(f"ranking epic: {ranking + [idx for idx in list(range(len(task.candidate_ids))) if idx not in ranking]}")
        return ranking + [idx for idx in list(range(len(task.candidate_ids))) if idx not in ranking]
    
    def _better(self, task: RankingTask, i: int, j: int) -> bool:
        return not self.oracle.sample_lt(
            task.query_id,
            task.candidate_ids[i],
            task.candidate_ids[j],
        )
...
class _HeapItem:
    ...
    def __lt__(self, other: "_HeapItem") -> bool:
        return self.ranker._better(self.task, self.idx, other.idx)
