...
each LLM call is a label query, and the systems question becomes which pairs to query to recover a strong top-$K$ within a budget.
...
Concretely, we ask whether replacing sorting-based PRP with active ranking yields practical efficiency gains in LLM reranking.
Since active rankers are designed to tolerate noisy pairwise feedback, our framing also enables a cheaper oracle:
we introduce a randomized-direction oracle that uses a single LLM call per pair by randomizing prompt order and canonicalizing outcomes, yielding direction-consistent win probabilities despite order sensitivity.
...
Pairwise Ranking Prompting (PRP) uses an LLM comparator and composes pairwise judgments into a ranking \citep{sun2023chatgptsearch,qin2024pairwise}.
Most PRP pipelines plug comparator into classical sorting, targeting a full order rather than a top-$K$ prefix under a call budget.
...
Our focus is orthogonal: within pairwise PRP, we study (i) noise-aware comparison scheduling (active ranking) and (ii) a cheaper single-call oracle.
...
are cost-optimal \citep{wisznia2025batchingcaching}.

\paragraph{Oracles for order sensitivity.}
LLM judgments can depend on which passage is shown first. For $\{i,j\}$ define
$\alpha_{ij}:=\Pr[i\succ j \mid (d_i,d_j)]$ and $\beta_{ij}:=\Pr[i\succ j \mid (d_j,d_i)]$.

\noindent\textbf{Bidirectional (two calls).}
Query both orders, canonicalize to ``$i$ vs.\ $j$'' (invert the second), and aggregate symmetrically.

\noindent\textbf{Randomized-direction (one call).}
Sample the order uniformly and canonicalize, inducing $p_{ij}=\tfrac12\alpha_{ij}+\tfrac12\beta_{ij}$ and hence $p_{ij}=1-p_{ji}$.
...
Bidirectional memoizes one canonical outcome per unordered pair (2 calls/pair), while randomized-direction samples one prompt order per call.
...
\emph{(ii)} Randomized-direction prompting compresses the ``time-to-quality'' for active ranking: Mohajer reaches near-peak quality by $B{=}200$ calls.
...
We now switch to randomized-direction prompting (one call per pair via random order + canonicalization) (Table~\ref{tab:budget_ndcg10_oracles_flan_xl}).

\noindent\textbf{Twice the pairs, same budget.} A key advantage of randomized-direction prompting is its query efficiency: because it issues a single LLM call per pair (versus two symmetric calls under bidirectional prompting), a budget of $B$ comparisons can reach $B$ distinct pairs, compared to only $B/2$ under bidirectional prompting. Consequently, for any fixed budget, randomized-direction gathers twice as much pairwise information, explaining its consistent gains across all rankers under budget constraints.
...
Figure~\textcolor{red}{[X]} plots NDCG@10 vs.\ elapsed time for the same methods/oracles as Table~\ref{tab:budget_ndcg10_oracles_flan_xl}.
The trend matches the budgeted view: active ranking reaches strong top-$K$ quality earlier (Mohajer plateaus by \textcolor{red}{[t\_plateau\_A100/H100/H200]}s), while sorting improves more gradually and may overtake only at long runtimes.
...
\par\vspace{2pt}\footnotesize\noindent\textit{Note.} Add a 1--2 sentence takeaway summarizing the trend across both years.
\end{table}
...
\par\vspace{2pt}\footnotesize\noindent\textit{Note.} Mohajer dominates BubbleSort when $k$ and the budget are low. As $k$ increases, the crossover point where BubbleSort overtakes Mohajer arrives sooner.
\end{table}
...
\par\vspace{2pt}\footnotesize\noindent\textit{Note.} Add a 1--2 sentence takeaway.
\end{table}
...
\par\vspace{2pt}\footnotesize\noindent\textit{Note.} Active-learning rankers are more robust to noisy comparisons and profit most from randomized-direction sampling. Classic sorting baselines either lose effectiveness (QuickSort, HeapSort) or require many more comparisons as cycles emerge under noise. A curious pattern is that sampling in BubbleSort improves performance: with cycles, borderline documents can swap stochastically, and when they meet a clearly stronger neighbor they get pushed downward, letting the latent transitive ordering surface more than with a deterministic oracle.
\end{table}
...
% ------------------------------------------------------------
% (AX) Qwen3-4B-Instruct: DL19/DL20 summary (ranker + oracle)
% Notes:
%  - "Sampling" is reported as "Randomized"
...
\textbf{DL20 NDCG@10 (\%)} &
\textbf{Avg. NDCG@10 (\%)} &
\textbf{Avg. Calls/Task} \\
\midrule
BubbleSort (Bidirectional) & 67.4 & \second{70.4} & 69.0 & 1,063 \\
BubbleSort (Randomized) & \best{71.6} & \best{72.5} & \best{72.1} & 3,669 \\
\midrule
HeapSort (Bidirectional) & 70.3 & 69.5 & 69.9 & 1,383 \\
HeapSort (Randomized) & 70.7 & 69.8 & \second{70.2} & 884 \\
\midrule
QuickSort (Bidirectional) & 70.7 & 68.8 & 69.8 & 1,567 \\
QuickSort (Randomized) & 69.7 & 69.2 & 69.5 & 571 \\
\midrule
Mohajer (Bidirectional) & 70.4 & 66.9 & 68.7 & 398 \\
Mohajer (Randomized) & \second{71.0} & 65.4 & 68.2 & \second{232} \\
\midrule
Mohajer + Bubble (Bidirectional) & 70.5 & 66.9 & 68.7 & 426 \\
Mohajer + Bubble (Randomized) & 70.3 & 67.6 & 68.9 & 347 \\
\midrule
PAC + Bubble (Bidirectional) & 64.2 & 67.1 & 65.7 & 333 \\
PAC + Bubble (Randomized) & 63.0 & 66.0 & 64.5 & \best{183} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\caption{
TREC DL2019/2020 results using \textbf{Qwen3-4B-Instruct}. “Randomized” corresponds to the sampling-direction oracle.
HeapSort corresponds to the PRP sorting baseline. Bold/underline indicate best/second-best per column (higher NDCG; lower calls).
}
\label{tab:qwen3_4b_dl19_dl20_summary}
\par\vspace{2pt}\footnotesize\noindent\textit{Note.} Add a 1--2 sentence takeaway (e.g., best effectiveness vs.\ best efficiency trade-off).
\end{table}
